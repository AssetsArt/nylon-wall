...
pub async fn collect(state: &Arc<AppState>) -> String {
    ...
    prom_gauge(
        &mut out,
        "nylon_wall_ebpf_loaded",
        "Whether eBPF programs are loaded (1=yes, 0=no)",
        ebpf,
    );
    ...
    prom_gauge(
        &mut out,
        "nylon_wall_rules_total",
        "Total firewall rules",
        rules,
    );
    prom_gauge(
        &mut out,
        "nylon_wall_nat_entries_total",
        "Total NAT entries",
        nat,
    );
    ...
    prom_gauge(
        &mut out,
        "nylon_wall_policies_total",
        "Total network policies",
        policies,
    );
    prom_gauge(
        &mut out,
        "nylon_wall_logs_total",
        "Total packet log entries",
        logs,
    );
    prom_gauge(
        &mut out,
        "nylon_wall_conntrack_entries",
    ...
    prom_gauge(
        &mut out,
        "nylon_wall_dhcp_pools_total",
        "Total DHCP pools",
        dhcp_pools,
    );
    prom_gauge(
        &mut out,
        "nylon_wall_dhcp_leases_active",
        "Active DHCP leases",
        dhcp_leases,
    );
    prom_gauge(
        &mut out,
        "nylon_wall_dhcp_clients_total",
        "Total DHCP WAN clients",
        dhcp_clients,
    );
